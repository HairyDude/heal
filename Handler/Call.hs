        ...
        argfields = formToAForm $ do
            let consolidate ::
                ...
                consolidate mresults = do
                    results <- mresults
                    let allresults = mconcat $ map (fmap return . fst) results
            ...
            consolidate $ forM args $ \(Arg name _atype opt) -> do
                let req = opt == Mandatory  -- Check later in case opt == FromKey
                    ...
                    mkfield :: (b -> APIArgument)
                                  ...
                                  -> FieldSettings App
                                  -> Maybe a
                                  -> (Maybe a -> FormResult b) -- post-processing
                                  -> MForm App App (FormResult APIArgument,
                                                    [FieldView App App])
                    mkfield constr fieldtype settings def process = do
                        (result, field) <- moptreq req fieldtype settings def
                        case result of
                            FormSuccess a  -> return (constr <$> process a, [field])
                ...
                case name of
                    ...
                    ArgTNames -> parseListField ArgNames
                                    ...
                                    characterName
                                    Nothing
                    ArgTCharacterID -> notImplemented   -- int
                    ArgTCorporationID -> notImplemented -- int
                    ArgTEventIDs -> parseListField ArgIDs
                                    "List of event IDs (comma-separated)"
                                    "Invalid event ID list"
                                    decimal
                                    Nothing
                    ArgTContractID -> notImplemented    -- int
